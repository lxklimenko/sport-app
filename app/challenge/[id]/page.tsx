...
export default async function ChallengePage({
  ...
  const teamRanking = await getTeamsRanking(challenge.id);
    ...
    <main className="min-h-screen bg-[#0D0F12] px-6 py-6 pb-24 text-[#F5F7FA]">
      <div className="mx-auto max-w-5xl">
        ...
        {teamRanking.length > 0 && (
          <section className="mt-6 rounded-[2rem] bg-[#171A1F] p-6 lg:p-8">
            <div className="flex items-center gap-3 mb-6">
              <Trophy className="w-5 h-5 text-[#C4EEDB]" />
              <h2 className="text-2xl font-semibold">Рейтинг команд</h2>
            </div>

            <div className="space-y-2">
              {teamRanking.slice(0, 10).map((tr, i) => {
                const medal = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : null;
                return (
                  <Link
                    key={tr.team.id}
                    href={`/teams/${tr.team.id}`}
                    className={`flex items-center gap-4 rounded-full px-5 py-4 transition hover:bg-white/10 ${
                      i < 3 ? "bg-white/5" : "bg-white/2"
                    }`}
                  >
                    <div className="w-8 text-center">
                      {medal ? (
                        ...
                        <span className="text-[#9AA0A6] font-semibold">{i + 1}</span>
                      )}
                    </div>

                    <div className="text-2xl">{tr.team.emoji}</div>

                    <div className="flex-1 min-w-0">
                      <div className="font-semibold truncate">{tr.team.name}</div>
                      <div className="text-xs text-[#9AA0A6]">
                        {tr.team.memberCount} {tr.team.memberCount === 1 ? "участник" : "участников"}
                      </div>
                    </div>

                    <div className="text-right">
                      <div className="font-semibold text-lg">
                        {tr.totalSteps.toLocaleString("ru-RU")}
                      </div>
                      <div className="text-xs text-[#9AA0A6]">{challenge.unitLabel}</div>
                    </div>
                  </Link>
                );
              })}
