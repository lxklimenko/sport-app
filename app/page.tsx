...
import { joinActiveChallengeAction } from "./actions";
...
export default async function Home() {
  ...
  const participantsCount = challenge ? await getParticipantsCount(challenge.id) : 0;
  const leaders = challenge ? await getLeaderboard(challenge.id, 3) : [];
  const alreadyJoined = userId && challenge ? await isParticipant(userId, challenge.id) : false;
    ...
    <div className="min-h-screen bg-[#131314] text-[#E3E3E3] font-sans pb-20">
      ...
      <section className="pt-32 pb-20 px-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-10 text-[#C4C7C5] text-lg">
          ...
          <span>{participantsCount} участников · активный челлендж</span>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          {!userId ? (
            ...
            <Link
              href="/profile"
              className="px-8 py-5 bg-[#C4EEDB] text-[#003829] rounded-full flex items-center gap-3 text-lg hover:bg-[#D5F5E4] transition"
            >
              Ты в челлендже — добавить шаги
              <ArrowRight className="w-5 h-5" />
            </Link>
          ) : (
            <form action={joinActiveChallengeAction}>
              <button
                type="submit"
                className="px-8 py-5 bg-[#A8C7FA] text-[#062E6F] rounded-full flex items-center gap-3 text-lg hover:bg-[#BBD6FE] transition cursor-pointer"
              >
                Вступить в челлендж
                <ArrowRight className="w-5 h-5" />
              </button>
          ...
          {challenge && (
            ...
            >
              <div>
                ...
                <h3 className="text-4xl mb-4">{challenge.title}</h3>
                <p className="text-lg opacity-80">
                  {challenge.description || `${challenge.days} дней непрерывного движения`}
                </p>
              </div>

              <div className="flex justify-between items-end mt-10">
                <div>
                  <div className="text-3xl">{participantsCount}</div>
      ...
      <section className="py-20 px-6 max-w-3xl mx-auto">
        ...
        {leaders.length === 0 ? (
          <div className="text-center py-10 text-[#C4C7C5]">
            <p className="text-lg">Пока никто не добавил шаги.</p>
            ...
            {leaders.map((leader, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{leader.totalSteps.toLocaleString()}</span>
                </div>
              );
            })}
          </div>
      ...
      <section className="py-20 flex justify-center">
        {!userId ? (
          ...
          <form action={joinActiveChallengeAction}>
            <button
              type="submit"
              className="px-12 py-6 bg-[#C4EEDB] text-black rounded-full flex items-center gap-4 hover:bg-[#D5F5E4] transition cursor-pointer"
            >
              Вступить
              <ArrowRight />
            </button>
